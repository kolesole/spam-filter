...
from FileHelper import PREDICTION_FNAME, TRUTH_FNAME
import email
import os, re
...
# Returns a tuple of (username, domain, TLD)
def parse_email_address(text:str) -> tuple[str, str, str] | None:
    for addr in text.split():
        if addr.count("@") != 1:
            continue
        username, domain_and_dns = addr.split('@')
        while not username[0].isalnum():
            username = username[1:]

        if domain_and_dns.count('.') == 0:
            continue
        domain, dns = domain_and_dns.split(".", maxsplit=1)
        while not dns[-1].isalnum():
            dns = dns[:-1]

        return username, domain, dns
    return